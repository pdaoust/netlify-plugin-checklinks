...
module.exports = {
  onPostBuild: async ({
    ...
    const regexRegex = /^\/[^/]+\/[dgimsuvy]*/;

    const skipPatternsAsRegex = skipPatterns.map((pattern) => regexRegex.test(pattern)
      ? new RegExp(pattern)
    ...
    const skipFragmentPatternsAsRegex = skipFragmentPatterns.map((pattern) => regexRegex.test(pattern)
      ? new RegExp(pattern)
      : new RegExp(RegExp.escape(pattern))
    );
    ...
    const todoPatternsAsRegex = todoPatterns.map((pattern) => regexRegex.test(pattern)
      ? new RegExp(pattern)
      : new RegExp(RegExp.escape(pattern))
    );
